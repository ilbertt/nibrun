...
// What Postgres raises when a second deployment reaches `deployments_running_idx` first.
function runningIndexViolation(): SQL.PostgresError {
  return new SQL.PostgresError('duplicate key value violates unique constraint', {
    code: 'ERR_POSTGRES_SERVER_ERROR',
    errno: '23505',
    constraint: 'deployments_running_idx',
  });
}
...
class FakeDeploymentsRepository implements DeploymentsRepositoryContract {
  ...
  activate(input: DeploymentByIdInput): Promise<DeploymentRow | null> {
    this.calls.push(input);
    if (this.#behaviour.activateError) {
      return Promise.reject(this.#behaviour.activateError);
    }
    return Promise.resolve(this.#behaviour.row ?? null);
  }
}
...
describe('one app asks for one deployment at a time', () => {
  ...
  test('losing the race to the running index is a conflict, not a 500', async () => {
    const { service } = serviceWith({ activateError: runningIndexViolation() });
  ...
  test('any other database failure is not disguised as one', async () => {
    ...
    const { service } = serviceWith({ activateError: boom });
