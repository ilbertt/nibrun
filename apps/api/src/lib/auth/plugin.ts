  ...
  return new Elysia({ name: 'auth' }).macro({
    auth: (identity: Identity) => ({
      async resolve({ request }) {
        ...
        // Said once, here: the column is null on every user from before there were strangers,
        // and a null is a person with an identity.
        const user = { ...session.user, isAnonymous: session.user.isAnonymous === true };
