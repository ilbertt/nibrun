...
describe('what a host measured of a guest is kept beside how full its filesystem is', () => {
  ...
  /**
   * The one guarantee that outlives every caller. Both readings are a subtraction on the guest's
   * side — total less free, total less available — and a subtraction is what produces a number
   * below zero when the two ends come from different units. Nothing upstream can write one of
   * these today; the point of a constraint is that nothing added later can either.
   */
  describe('a reading the guest could not have taken is refused by the table', () => {
    function writeDirectly({
      appId,
      columns,
      values,
    }: {
      ...
      return sql.unsafe(
        `INSERT INTO nibrun.app_usage (app_id, ${columns}) VALUES ($1::uuid, ${values})`,
        [appId],
      );
    }

    test('memory spent beyond what the guest has is not a reading', async () => {
      const appId = await createApp('impossible-memory');

      await expect(
        writeDirectly({
          appId,
          columns: 'memory_total_bytes, memory_used_bytes, compute_measured_at',
          values: `100, 101, '${EARLIER}'`,
        }),
      ).rejects.toMatchObject({ constraint: 'app_usage_memory_within_itself' });
    });

    test('memory spent below nothing is not a reading either', async () => {
      const appId = await createApp('negative-memory');

      await expect(
        writeDirectly({
          appId,
          columns: 'memory_total_bytes, memory_used_bytes, compute_measured_at',
          values: `100, -1, '${EARLIER}'`,
        }),
      ).rejects.toMatchObject({ constraint: 'app_usage_memory_within_itself' });
    });

    test('a volume fuller than it is holds to the same rule', async () => {
      const appId = await createApp('impossible-volume');
      ...
      ).rejects.toMatchObject({ constraint: 'app_usage_volume_within_itself' });
    });
    ...
    test('a share of more than every vCPU is not a share', async () => {
      const appId = await createApp('impossible-share');

      await expect(
        writeDirectly({
          appId,
          columns: 'memory_total_bytes, memory_used_bytes, cpu_share, compute_measured_at',
          values: `100, 10, 1.5, '${EARLIER}'`,
        }),
      ).rejects.toMatchObject({ constraint: 'app_usage_cpu_share_is_a_share' });
    });
    ...
    test('a family written without its moment is half a reading', async () => {
      const appId = await createApp('half-a-reading');
      ...
      ).rejects.toMatchObject({ constraint: 'app_usage_compute_whole' });
    });
  });
