...
a `@notNull` marker in a column comment for `bun-sqlgen` to type it as always
present — declared once in the same migration rather than per query:

```sql
COMMENT ON COLUMN nibrun.app.created_at IS 'Derived from the uuidv7 id; the moment the row was created. @notNull';
```
