...
A view's columns inherit the `@type` and the prose of the base column each one
passes through, so never restate either on the view. Nullability is what does not
carry: the catalog knows nothing of the view's own `WHERE`, so a column the view
filters to non-null takes a `@notNull` comment of its own — on the view column,
not as a pragma in every query that selects it.

Row types are generated by `@ilbertt/bun-sqlgen` into `src/db/queries.gen.ts`
...
The same file exports a `schema` value naming every relation, column, index and
constraint. Code that has to name one — matching a unique violation against the
index it came from — reads it from there, so a rename in a migration breaks the
build instead of the behaviour.
