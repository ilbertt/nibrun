...
import { initialDataFrom } from '#lib/initial-data.ts';
...
export const command = defineCommand('run [command]', {
  description:
    'Deploy a compiled binary and run it, from this machine or from an https url nibrun fetches it at. Quote the binary with its arguments to pass them on: nib run "./my-server serve --port 8080". Every option a deploy takes is listed by nib run anything --help — any word will do in place of the binary, because the help does not read it.',
  params: {
  ...
  options: {
    ...
    [DATA_FOLDER_FLAG]: {
      schema: z.string().optional(),
      description:
        "A folder on this machine the app's data/ is created holding, or a .tar.gz or .zip already holding it. A folder is packed here; either way it is the root of the archive that becomes the root of data/, so a folder is not itself a directory inside it. Only for an app being created: the data of one that already exists was created when it was.",
    },
