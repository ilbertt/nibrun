...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'panel-face group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-[min(var(--radius-4xl),24px)] border-2 border-border bg-card py-(--card-spacing) text-card-foreground text-sm shadow-sm [--card-spacing:--spacing(5)] has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(4)] *:[img:first-child]:rounded-t-[min(var(--radius-4xl),24px)] *:[img:last-child]:rounded-b-[min(var(--radius-4xl),24px)]',
        className,
      )}
      {...props}
    />
